...
class BlogController extends Controller
{
    ...
    public function oneBlog($slug){
        try{
            ...
            //$oneBlog = $blogdata->blog_posts_select_slug($slug);
            $oneBlog = $blogdata->blog_posts_select_slug_and_blog_id($slug,$blogID);
            ...
            //$folderImageByBlog = asset("imageBlog/".$oneBlog['blog_posts_id']."/");
            ...
            //$oneBlog['categoryOfBlog'] = implode(", ",$allCategoryName);
            ...
            /*
            $arrayResultSuggestion = [];
            foreach($oneBlog['catogory_id_blog'] as $oneBlog_categorie_id){
                $blogByOneCategery = $blogdata->blog_posts_select_pourunblog_et_unecategorie($oneBlog['blog_id'],$oneBlog_categorie_id);
                foreach($blogByOneCategery as $oneBlogByOneCategory){
                    array_push($arrayResultSuggestion,$oneBlogByOneCategory);
                }
            }
            dd($arrayResultSuggestion);
            */
            //blog_categorie_id
            /*if(is_dir($folderImageByBlog)){
                $allImage = scandir($folderImageByBlog);
                foreach($allImage as $oneImage){
                    if(str_contains("1000x700.webp",$oneImage)){
                        $oneBlog['image'] = $folderImageByBlog."/".$oneImage;
                    }
                }
              }
            */
    ...
    public function allBlog($blogID){
        try{
            ...
            /*
            $sql = "SELECT *,(
                                SELECT GROUP_CONCAT( `category_name` SEPARATOR ',' ) FROM blog_posts 
            ...
            (
                                SELECT GROUP_CONCAT( `blog_categories.slug` SEPARATOR ',' ) FROM blog_posts 
                                left outer join  blog_post_categories on blog_post_categories.blog_post_id = blog_posts.blog_posts_id
                                left outer join blog_categories on blog_post_categories.blog_categorie_id= blog_categories.blog_categorie_id
            ) as arrayblogCategoriesSlug 
            FROM blog_posts
            WHERE areaproduction=1 order by posted_at DESC;";
            */
            //$blogID = 1;
            ...
            $queryRecords=$blogdata->ExecuteRequeteDepuisResponse($sql);
